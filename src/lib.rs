...
mod traits {
        ...
        >
    {
        pub fn load_from_buffer(buf: &'a mut [u8]) -> Self {
            let (header_slice, allocator_slice) =
                buf.split_at_mut(std::mem::size_of::<HeaderType>());
            let header = HeaderType::load_mut_bytes(header_slice).expect("Failed to load header");
            let allocator = SimpleNodeAllocator::load_mut_bytes(allocator_slice)
                .expect("Failed to load allocator");
            Self {
                header,
                allocator,
